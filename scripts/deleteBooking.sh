...
ARCHIVO_DE_REGISTRO="${DIRECCION_EVIDENCIA}/deleteBooking ${MARCADETIEMPO}.log"
...
log "================================================"
log "  Restful-Booker DeleteBooking Test"
...
log "----------Inicia a jecutar el test: DeleteBooking---------------"
if [ ! -z "$TOKEN" ]; then
    if [ ! -z "$FIRST_BOOKING_ID" ] ; then       
        ...
        if [ "$HTTP_CODE" -eq 201 ]; then
            ...
            RESPONSE_CODE=$(echo "$RESPONSE_BOOKING_BY_ID_CODE" | tail -n1)
            RESPONSE_BODY=$(echo "$RESPONSE_BOOKING_BY_ID_CODE" | sed '$d')              
            
            if  [ "$RESPONSE_CODE" -eq 404 ] && echo "$RESPONSE_BODY" | grep -qi "Not Found"; then
                imprimir_resultado 0 "DeleteBooking eliminó correctamente la reserva con ID $FIRST_BOOKING_ID"
            else
                imprimir_resultado 1 "DeleteBooking falló al eliminar la reserva con ID $FIRST_BOOKING_ID"
            fi           
        else
            imprimir_resultado 1 "DeleteBooking failed (HTTP $HTTP_CODE)"
        fi
    else
        imprimir_resultado 1 "No hay ID de reserva disponible para DeleteBooking"
    fi
else    
